...
//            }
            ...
            if (isIncludeTop() && !canChildScrollUp() && top > 0) {
                final int topBound = getPaddingTop();
                final int bottomBound = verticalDragRange;
                result = Math.min(Math.max(top, topBound), bottomBound);
                dragEdge = DragEdge.TOP;
            } else if (isIncludeBottom() && !canChildScrollDown() && top < 0) {
                final int topBound = -verticalDragRange;
                final int bottomBound = getPaddingTop();
                result = Math.min(Math.max(top, topBound), bottomBound);
...
//            }
            ...
            if (isIncludeLeft() && !canChildScrollRight() && left > 0) {
                final int leftBound = getPaddingLeft();
                final int rightBound = horizontalDragRange;
                result = Math.min(Math.max(left, leftBound), rightBound);
                dragEdge = DragEdge.LEFT;
            } else if (isIncludeRight() && !canChildScrollLeft() && left < 0) {
                final int leftBound = -horizontalDragRange;
                final int rightBound = getPaddingLeft();
                result = Math.min(Math.max(left, leftBound), rightBound);
    ...
    public enum Direction {
        ...
        TOP(0x0001), BOTTOM(0x0010), LEFT(0x0100), RIGHT(0x1000), VISIBLE(0x0011), HORIZONTAL(0x1100);
    ...
    private Direction direction = Direction.VISIBLE;
    ...
    private void addDirection(Direction dir) {
        direction.setValue(direction.getValue() | dir.getValue());
    }

    private boolean isIncludeTop() {
        return (direction.getValue() & Direction.TOP.getValue()) == Direction.TOP.getValue();
    }

    private boolean isIncludeBottom() {
        return (direction.getValue() & Direction.BOTTOM.getValue()) == Direction.BOTTOM.getValue();
    }

    private boolean isIncludeLeft() {
        return (direction.getValue() & Direction.LEFT.getValue()) == Direction.LEFT.getValue();
    }

    private boolean isIncludeRight() {
        return (direction.getValue() & Direction.RIGHT.getValue()) == Direction.RIGHT.getValue();
    }
}